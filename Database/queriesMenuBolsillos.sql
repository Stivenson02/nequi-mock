...
UPDATE `pockets`
SET `saved_money` = `saved_money` - #{withdrawn_money_from_pocket}
WHERE `storage_id` = #{pocket_storage_id};

UPDATE `savings_accounts`
SET `available_money` = `available_money` + #{withdrawn_money_from_pocket}
WHERE `storage_id` = #{savings_account_storage_id};

INSERT INTO `transactions`(`source_storage_id`, `destination_storage_id`, `money_transferred`)
VALUES(#{pocket_storage_id}, #{savings_account_storage_id}, #{withdrawn_money_from_pocket});
